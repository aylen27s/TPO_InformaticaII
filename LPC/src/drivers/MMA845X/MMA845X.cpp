...
//extern Gpio LED_VERDE;
extern UART0 Uart0;
uint8_t data[192], CantDatos;
uint8_t buffer[100];

static uint32_t bpm_buffer[BPM_BUFFER_SIZE] = {0};
static uint8_t bpm_buffer_index = 0;
static uint8_t buffer_full = 0; // un flag para saber si se llenó el buffer

volatile uint8_t g_flagLecturaMAX30102 = 0;

//Variable para guardar la muestra una vez que pasa por el filtro
int32_t MAX30102ProcessSample = -1;
...
void MAX30102( void )
{
	...
	case 1:
		...
		if( GetFlagLecturaMAX30102() )
		{
			...
			{
				bpm_buffer[bpm_buffer_index] = bpm_del_ciclo;
				bpm_buffer_index++;
				if(bpm_buffer_index >= BPM_BUFFER_SIZE){
					bpm_buffer_index = 0;
					buffer_full = 1; // marcamos que el buffer se lleno una vez
				}

				// si el buffer está lleno, se calcula el promedio
				if(buffer_full){
					uint32_t bpm_sum = 0;
...
uint8_t MAX30102InitialConfig( void )
{
	...
	case 4:
		if( !GetDemora_IIC() )
			{
				MAX30102_Write( MAX30102_LED1_PA, 0x15);		//
				SetDemora_IIC(IIC_DELAY_ms);
				estado = 5;
			}
			break;
	...
	case 5:
		if( !GetDemora_IIC() )
		{
			MAX30102_Write( MAX30102_LED2_PA, 0x0F);		//
			SetDemora_IIC(IIC_DELAY_ms);
			estado = 6;
		}
		break;
...
int32_t MAX30102_GetLectura(void){
	//Retorna -1 si no hubo muestra válida procesada o si el sensor aun no refrescó la lectura, o >0 si llegó a procesarse la nueva muestra.
	return MAX30102ProcessSample;
}

void MAX30102_CleanBuffer(void){
	MAX30102ProcessSample = -1;
}
